...
// @updateURL    https://raw.githubusercontent.com/marketozk/ozon-seller-scripts/master/tools/ozon_toolbox.user.js
// @downloadURL  https://raw.githubusercontent.com/marketozk/ozon-seller-scripts/master/tools/ozon_toolbox.user.js
// ==/UserScript==
...
if __name__ == "__main__":
    ...
    const API = {
        ...
        COURIER_SCHEDULE_SET: '/api/delivery-method-service/delivery-method/courier-schedule/set',
    ...
    function loadConfig() {
        try {
            ...
            return {
                ...
                stock: { ...DEFAULT_CONFIG.stock, ...(parsed.stock || {}) }
            };
        } catch {
    ...
    function saveConfig(partial) {
        ...
        const merged = {
            ...
            stock: { ...current.stock, ...(partial.stock || {}) }
        };
    ...
    const WarehouseModule = {
        ...
        // Генерация телефона
        generatePhone() {
            // Реальные мобильные коды России: 9XX (900-999)
        ...
        // Генерация полигона (неровный круг для естественного вида)
        generateCirclePolygon(centerLat, centerLng, radiusKm, points = 24) {
            ...
            for (let i = 0; i < points; i++) {
                ...
                // Добавляем случайное отклонение ±15% для неровности
                const randomFactor = 0.85 + Math.random() * 0.30; // от 0.85 до 1.15
                const adjustedRadius = radiusKm * randomFactor;
                const dLat = (adjustedRadius / earthRadius) * Math.cos(angle) * (180 / Math.PI);
                const dLng = (adjustedRadius / earthRadius) * Math.sin(angle) * (180 / Math.PI) / Math.cos(centerLat * Math.PI / 180);
        ...
        async run(config) {
            ...
            notify(0, 9, 'Запуск', `Адрес: ${warehouseAddress.substring(0, 40)}...`, 'info');
            
            try {
                // ШАГ 1: Геокодирование
                if (this.shouldStop) throw new Error('Остановлено');
                logWh('Шаг 1/8: Геокодирование адреса...');
                notify(1, 9, 'Геокодирование', 'Определяем координаты адреса...');
                ...
                notify(1, 9, 'Координаты ✓', `${this.state.lat.toFixed(4)}, ${this.state.lng.toFixed(4)}`, 'success');
                await delay(2000);
                
                // ШАГ 2: Создание черновика склада
                if (this.shouldStop) throw new Error('Остановлено');
                logWh('Шаг 2/9: Создание черновика склада...');
                notify(2, 9, 'Черновик', 'Создаём склад...');
                
                // Извлекаем город из адреса если геокодер не вернул
                let cityName = this.state.parsedAddress.city;
                if (!cityName) {
                    // Пробуем извлечь город из адреса (формат: "..., г Город, ..." или "..., Город, ул...")
                    const cityMatch = warehouseAddress.match(/(?:г\.?\s*|город\s+)([^,]+)/i) ||
                                      warehouseAddress.match(/,\s*([^,]+?)\s*,\s*(?:ул|улица|пр|проспект|пер|переулок)/i);
                    if (cityMatch) {
                ...
                notify(2, 9, 'Черновик ✓', `ID: ${this.state.warehouseDraftId}`, 'success');
                await delay(3000);
                
                // ШАГ 3: Создание метода доставки
                if (this.shouldStop) throw new Error('Остановлено');
                logWh('Шаг 3/8: Создание метода доставки...');
                notify(3, 9, 'Метод доставки', 'Создаём Express метод...');
                ...
                notify(3, 9, 'Метод ✓', `ID: ${this.state.methodId}`, 'success');
                await delay(3000);
                
                // ШАГ 4: Создание зоны доставки
                if (this.shouldStop) throw new Error('Остановлено');
                logWh('Шаг 4/8: Создание зоны доставки...');
                notify(4, 9, 'Зона доставки', 'Создаём область...');
                ...
                notify(4, 9, 'Зона ✓', `ID: ${this.state.areaId}`, 'success');
                await delay(2000);
                
                // ШАГ 5: Создание полигона
                if (this.shouldStop) throw new Error('Остановлено');
                logWh('Шаг 5/8: Создание полигона...');
                notify(5, 9, 'Полигон', 'Создаём область покрытия...');
                ...
                notify(5, 9, 'Полигон ✓', `Радиус: ${radiusKm} км`, 'success');
                await delay(1500);
                
                // ШАГ 6: Привязка полигона к зоне
                if (this.shouldStop) throw new Error('Остановлено');
                logWh('Шаг 6/8: Привязка полигона к зоне...');
                notify(6, 9, 'Привязка', 'Связываем полигон с зоной...');
                ...
                notify(6, 9, 'Привязка ✓', 'Полигон связан', 'success');
                await delay(2000);
                
                // ШАГ 7: Привязка склада и настройка возвратов
                if (this.shouldStop) throw new Error('Остановлено');
                logWh('Шаг 7/8: Привязка склада и настройка возвратов...');
                notify(7, 9, 'Настройки', 'Привязываем склад, настраиваем возвраты...');
                ...
                notify(7, 9, 'Настройки ✓', 'Склад привязан', 'success');
                await delay(2000);
                
                // ШАГ 8: Установка расписания курьера (такое же как у склада)
                if (this.shouldStop) throw new Error('Остановлено');
                logWh('Шаг 8/9: Установка расписания курьера...');
                notify(8, 9, 'Расписание', 'Настройка графика работы...');
                
                await apiRequest(API.COURIER_SCHEDULE_SET, {
                    method: 'POST',
                    body: JSON.stringify({
                        company_id: companyId,
                ...
                logWh('✓ Расписание курьера установлено');
                notify(8, 9, 'Расписание ✓', 'График работы настроен', 'success');
                await delay(1500);
                
                // ШАГ 9: Активация
                if (this.shouldStop) throw new Error('Остановлено');
                logWh('Шаг 9/9: Активация метода доставки...');
                notify(9, 9, 'Активация', 'Запускаем склад...');
                ...
                notify(9, 9, 'ГОТОВО! 🎉', `Склад ID: ${this.state.warehouseId}`, 'success');
                showToast('🎉 Склад создан!', 'success');
                
            } catch (error) {
                logWh(`❌ Ошибка: ${error.message}`);
                logWh(`Состояние: ${JSON.stringify(this.state)}`);
                notify(0, 9, 'ОШИБКА ❌', error.message.substring(0, 60), 'error');
    ...
    const StockModule = {
        ...
        async run(config) {
            ...
            const notify = (step, total, title, message, type = 'info') => {
                NotificationSystem.show({
                    title: `📦 Остатки [${step}/${total}]: ${title}`,
                    message: message,
                    type: type,
                    duration: type === 'error' ? 8000 : 4000
                });
